...
const { firstDefined: firstDefined$1, style: style$1 } = utils;
...
function UINumber(config) {
	...
	var xstep = firstDefined$1(config.xstep, config.step, 0.001);
	var ystep = firstDefined$1(config.ystep, config.step, 0.1);
	var wheelStep = firstDefined$1(config.wheelStep, ystep);
	var wheelStepFine = firstDefined$1(config.wheelStepFine, xstep);
...
const { STORAGE_PREFIX: STORAGE_PREFIX$1, style: style$3 } = utils;
...
function LayerCabinet(data, dispatcher) {
	...
	style$3(play_button.dom, button_styles, { marginTop: '2px' } );
	...
	style$3(stop_button.dom, button_styles, { marginTop: '2px' } );
	...
	style$3(undo_button.dom, op_button_styles);
	undo_button.onClick(function() {
		dispatcher.fire('controls.undo');
	});

	var redo_button = new IconButton(16, 'repeat', 'redo', dispatcher);
	style$3(redo_button.dom, op_button_styles);
	...
	style$3(file_open.dom, op_button_styles);
	...
	style$3(save.dom, op_button_styles);
	...
	style$3(save_as.dom, op_button_styles);
	...
	style$3(download_alt.dom, op_button_styles);
	...
	style$3(upload_alt.dom, op_button_styles);
...
const proxy_ctx$1  = utils.proxy_ctx;
...
var style$4 = utils.style;
var saveToFile$1 = utils.saveToFile;
var openAs$1 = utils.openAs;
var STORAGE_PREFIX$2 = utils.STORAGE_PREFIX;
...
function Timeliner(target) {
	...
	*/
	...
	function save(name) {
		...
		try {
			localStorage[STORAGE_PREFIX$2 + name] = json;
	...
	function open(title) {
		if (title) {
			loadJSONString(localStorage[STORAGE_PREFIX$2 + title]);
		}
	}
	...
	*/
	...
	style$4(pane_title, header_styles, {
		borderBottom: '1px solid ' + Theme.b,
		textAlign: 'center'
	});
	...
	style$4(top_right_bar, header_styles, {
		textAlign: 'right'
	});
	...
	style$4(resize_full.dom, button_styles, { marginRight: '2px' });
	...
	style$4(pane_status, footer_styles, {
	...
	style$4(bottom_right, footer_styles, {
		textAlign: 'right'
	});
	...
	style$4(plus.dom, button_styles);
	...
	trash.onClick(function() {
		var name = data.get('name').value;
		if (name && localStorage[STORAGE_PREFIX$2 + name]) {
			var ok = confirm('Are you sure you wish to delete ' + name + '?');
			if (ok) {
				delete localStorage[STORAGE_PREFIX$2 + name];
				dispatcher.fire('status', name + ' deleted');
				dispatcher.fire('save:done');
			}
		}
	});
	style$4(trash.dom, button_styles, { marginRight: '2px' });
